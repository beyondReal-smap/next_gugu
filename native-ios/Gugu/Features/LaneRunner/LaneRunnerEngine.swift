...
// 구구 레인 진행 상태 관리 (LaneRunnerScreen.tsx 의 상태/루프 부분 이식)
// 게임 규칙은 Core/LaneRunner.swift 순수 함수가 담당하고, 여기서는 프레임 루프와 기록 저장만 맡는다.

@Observable
final class LaneRunnerEngine {
    private(set) var game: LaneRunnerState
    ...
    init() {
        self.table = nil
        self.game = LaneRunner.create(table: nil, phase: .ready)
        self.bests = LaneRunnerEngine.loadBests()
    }

    // MARK: - 기록

    private static func loadBests() -> [String: Int] {
        let raw = Persistence.load([String: Int].self, key: Persistence.laneBestKey) ?? [:]
    ...
    private func recordBestIfNeeded() {
        ...
        Persistence.save(bests, key: Persistence.laneBestKey)
    }
    ...
    func move(_ delta: Int) {
        let before = game.lane
        game = LaneRunner.move(game, delta: delta)
        if game.lane != before { Haptics.selection() }
    }

    func setLane(_ lane: Int) {
        let before = game.lane
        game = LaneRunner.setLane(game, lane: lane)
        if game.lane != before { Haptics.selection() }
    }
    ...
    private func tick() {
        ...
        let before = game
        game = LaneRunner.advance(game, dt: dt)

        if before.outcome != .correct && game.outcome == .correct {
            Sound.shared.correct()
            Haptics.impact(.light)
            recordBestIfNeeded()
        }
        if before.outcome != .wrong && game.outcome == .wrong {
            Sound.shared.wrong()
            Haptics.error()
        }
        if before.outcome != .hit && game.outcome == .hit {
            Haptics.error()
        }
        if game.phase == .over {
            recordBestIfNeeded()
